    ...
    <section className="ws-surface shrink-0 overflow-hidden rounded-xl" aria-label={t('axisSettings')}>
      {compactViewport ? (
        <>
          <button
            type="button"
            className="flex w-full items-center justify-between px-3 py-2 text-left text-sm font-medium text-[var(--ws-ink)]"
            onClick={toggleMobile}
            aria-expanded={mobileExpanded}
          >
            <span className="flex items-center gap-2"><SlidersHorizontal className="size-4 text-primary" />{t('axisSettings')}</span>
            <ChevronDown className={`size-4 text-primary transition-transform ${mobileExpanded ? 'rotate-180' : ''}`} />
          </button>
          {mobileExpanded && <div className="border-t border-[var(--ws-border)] p-3">{fields}</div>}
        </>
      ) : (
