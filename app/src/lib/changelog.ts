...
export const CHANGELOG_ENTRIES: ChangelogEntry[] = [
  {
    ...
    changes: [
      ...
      { zh: '跨应用复制按选中内容导出波形和文字：复制到 PowerPoint 后的波形可直接编辑；Visio 复制后暂不支持编辑，如需编辑请使用“导出 SVG”；不包含网格、坐标轴、图例或 Cursor。', en: 'Cross-application copy exports only selected waveforms and text: pasted waveforms are directly editable in PowerPoint; Visio does not support editing after copy, so use “Export SVG” when editing is required. Grid, axes, legend, and Cursor are excluded.' },
