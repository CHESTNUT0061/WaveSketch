...
function App() {
  ...
  const toggleZoomOverlay = useCallback(() => {
    setZoomOverlayCollapsed((collapsed) => {
      ...
      try {
        localStorage.setItem(ZOOM_OVERLAY_COLLAPSED_KEY, String(next));
      } catch {
        // Ignore unavailable storage (for example private or restricted contexts).
      }
      return next;
    });
  }, []);
  ...
  return (
    <div className="ws-app-shell flex h-dvh min-h-0 w-full flex-col overflow-hidden bg-[var(--ws-cream)] text-[var(--ws-ink)]">
      ...
      <div className="ws-surface mx-2 mb-2 flex shrink-0 flex-col gap-1 rounded-xl p-1.5 lg:mx-4 lg:flex-row lg:items-center lg:gap-2">
          <div className="ws-command-row ws-scroll-fade min-w-0 w-full lg:w-auto lg:flex-1" aria-label={t('drawingTools')}>
